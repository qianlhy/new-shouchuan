...
import com.diy.dto.OrderAddressUpdateDTO;
...
//    }
    ...
    @ApiOperation("修改订单地址（未发货前可修改）")
    public Result updateOrderAddress(
            @ApiParam(value = "订单ID", required = true) @PathVariable Long id,
            @Valid @RequestBody OrderAddressUpdateDTO addressDTO) {
        log.info("修改订单地址，订单ID: {}, 地址信息: {}", id, addressDTO);
        orderService.updateOrderAddress(id, addressDTO);
        return Result.success();
    }
}
