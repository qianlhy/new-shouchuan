...
const visualRadius = computed(() => {
  ...
  const circumferenceRpx = circumferenceMm * BEAD_MM_TO_RPX
...
function getBeadStyle(bead, index) {
  ...
  const size = Number(bead.size || 8) * BEAD_MM_TO_RPX
  const radius = layoutRadius.value
  ...
  // 阈值：24mm * 7 = 168rpx
  const zIndex = size >= (24 * BEAD_MM_TO_RPX) ? 1 : 10
...
async function generateDesignImage() {
  ...
  try {
    ...
    const drawBead = (b, i, layout) => {
        const angle = layout.angle
        const beadSizeRpx = Number(b.size || 8) * BEAD_MM_TO_RPX
...
.size-opt.selected { background: #F7F4FC; border-color: #6B4EFF; }
...
.pv-price { font-size: 26rpx; color: #6B4EFF; font-weight: 600; }
.preview-total {
  display: flex;
  justify-content: space-between;
  align-items: center;
...
.pv-total-num { font-size: 36rpx; color: #6B4EFF; font-weight: 700; }
.preview-actions {
  display: flex;
  gap: 16rpx;
...
.act-submit { background: linear-gradient(135deg, #6B4EFF, #8B6CFF); color: #fff; }
...
.demo-bead-mirror {
  width: 40rpx;
  height: 40rpx;
  background: linear-gradient(90deg, #C4B5FD 50%, #6B4EFF 50%);
  border-radius: 50%;
